...
| "Seal document with forensic report" stops at the Forensic Scan step with "No forensic service at <host> — forensic report not produced" and an alert | The address the page was opened from has no API (the old Pages copy answers every POST with 405; the March mock-up answers with its shell). Since 2026-09-11 forensic mode pre-flights `/api/v1/site/health` and refuses to seal an unreviewed forensic report. | `voPreflightForensicService` (ENGINE.md §12.10); open the Worker address or choose Seal document |
...
| The browser asks "This bundle has N more scanned pages beyond the standard on-device OCR limit… Read them all now?" | The bundle has more image-only pages than `VO_OCR_MAX_PAGES`; the person sealing decides once. Cancel keeps the limit and the report names the unread pages on its cover. | `voOcrAskToContinue` (§12.10) |
| The cover says "AI REVIEW NOT RUN" or "INCOMPLETE READ: N pages not read", and section 4's Review column says NOT REVIEWED | True statements: the advisory AI review did not run (service unreachable or model down), or pages were left unread. Not a bug — the report is honest about its own limits. | §12.10 item 4; do not relabel |
