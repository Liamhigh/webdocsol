...
| A finding's evidence ends "[OCR page: severity held at Low until the quoted characters are verified against the page image]" | Every page the finding cites was OCR-recovered and the finding is a character-sensitive check (registration, VAT, identity or account number, section number, definition, figure, date, signature mark). | `voCapOcrFormatFindings` (§12.10); read the page image, then judge |
